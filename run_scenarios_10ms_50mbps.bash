...
export OUTPUTDIR="$(pwd)/x86_res/sequential/10ms_50mbps"
...
do
	...
	do
		...
		then
			echo "Being safe..."
			cp tmux-run-docker-part2_safe.bash build/tmux-run-docker-part2.bash
		fi
		if [[ $ALG == DILITHIUM2 ]]
		then
			if [[ $UDPSIZE -eq 256 ]]
		...
		./run_exps_10ms_50mbps.bash 0 1000 | tee scratch.log
